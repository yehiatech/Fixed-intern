...
2. Tool-calling -> Bedrock `converse` + 4 tools; model picks which to call.
3. Citation     -> similarity >= 0.75 = KB-grounded; else labeled fallback.
4. Logging      -> every interaction saved to chat_interactions.
...
def _tool_check_order_status(tool_input: dict, organization_id: str) -> dict:
 return {
"available": False,
"message": "خدمة تتبع الطلبات غير متاحة حاليًا.",
}

def _tool_escalate_to_human(tool_input: dict, organization_id: str) -> dict:
 return {
"escalated": True,
"message": "تم تصعيد هذه المحادثة لمراجعة أحد موظفي الدعم البشري.",
}
...
query: str,
organization_id: str
) -> tuple[str, float | None, bool, dict | None]:
    ...
    for block in output_message["content"]:
        ...
        if name == "search_knowledge_base":
            ...
            if results:
                ...
                if best["similarity"] >= CITATION_THRESHOLD:
                    return (
                        best["chunk_text"],
                        best["similarity"],
                        True,
                ...
                if (
                    best_kb_similarity is None
                    or best["similarity"] > best_kb_similarity
                ):
...
if results and results[0]["similarity"] >= CITATION_THRESHOLD:
    best_kb_result = results[0]
    answer = best_kb_result["chunk_text"]
    best_similarity = best_kb_result["similarity"]
    used_kb_tool = True
else:
...
if (
    ...
    and best_similarity >= CITATION_THRESHOLD
    and best_kb_result
):
    ...
    answer = (
        f"{answer}\n\n"
        f"المصدر: {best_kb_result['source_file']} "
        f"- الصفحة {best_kb_result['page_number']}"
    )
else:
    source_type = "fallback_general"
    answer = (
        f"[إجابة من الذكاء الاصطناعي، وليست من قاعدة المعرفة] "
        f"{answer}"
    )
