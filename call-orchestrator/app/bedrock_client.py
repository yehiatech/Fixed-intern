"""
Runs one full "agent turn" — send the conversation to Bedrock, execute
any tool it calls, send the result back, repeat until Bedrock returns
plain text to speak. Restored unchanged from the pre-LiveKit T-12
version — this part of the pipeline never cared whether the phone leg
was Twilio or Vonage, only the webhook wiring around it changes.

DEV_MODE: if no AWS credentials are configured, falls back to a simple
keyword-based fake responder so the webhook flow can be tested locally
without needing real Bedrock model access.
"""
import logging
...
def _fake_agent_turn(call_id: str, messages: list[dict]) -> tuple[str, str]:
    """DEV_MODE ONLY. Simple keyword matching so the full webhook/NCCO
